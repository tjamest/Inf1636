...
public class Jogo implements MouseListener {
	...
	public static boolean newgame = false, jogadaNormal = false, concluiuJogada = false,
			 selecionado = false , captura = false;
//	public static boolean ehAbrigo = false;
	...
	public static int checaSeAbrigo(int x, int y) {// checa se casa dada � ABRIGO
		for(int i=0;i<4;i++) { 
	...
	public static int checaSeSaida(int x, int y) { // checa se � casa de SAIDA
		for(int i=0;i<4;i++) {
	...
	public static int checaSeBarreira(int x, int y) {// checa se � BARREIRA
		for (int i = 0; i<64; i++) { 
	...
	public static int checkMovimentAndUpdate(int x, int y, Peao peca) { // retorna o tipo da casa 1-abr 2-sai 3-barr 0- podemover
		...
		if (index2Click != -1) {
			...
			if (abrigo[index2Click][0] != null && abrigo[index2Click][1] != null) {
				System.out.printf("J� tem duas pecas no vetor de abrigos com esse index \n");
				System.out.printf("N�o pode mover \n");
				retornar = 1;
			}else {
				if (abrigo [index2Click][0] == null) {
					System.out.printf("Preenchedo casa [%d][0] com peca de cor %s\n",index2Click, peca.corP );
					...
					if(abrigo [index2Click][1] == null){
						System.out.printf("Preenchedo casa [%d][1] com peca de cor %s\n",index2Click, peca.corP );
			...
			if(index2Click != -1) {
				...
				if (saida[index2Click][0] != null && saida[index2Click][1] != null) {
					System.out.printf("J� tem duas pecas no vetor de casas de saida com esse index \n");
					System.out.printf("N�o pode mover \n");
					retornar = 2;
				}else {
				...
				if (retornar != 0)
					...
					if(index1Click != -1) {
						if (saida[index1Click][0] == peca && saida[index1Click][1]!= null) {
							System.out.printf("Entrou 1 if\n");
							saida[index1Click][0] = saida[index1Click][1];
							saida[index1Click][1] = null;
							saida[index1Click][0].alteraTam = false;
							System.out.printf("2 virou 1 \n");

						}
						else {
							System.out.printf("Entrou 2 if\n");

							saida[index1Click][0] = null;
						}
					}
				}
				...
				if(index2Click != -1) {
					...
					if (barreira[index2Click][0] != null && barreira[index2Click][1] != null) {
						System.out.printf("J� tem duas pecas no vetor de casas de barreira com esse index \n");
						System.out.printf("N�o pode mover \n");
						retornar = 3;
					}else {
						...
						else if(barreira [index2Click][0] != null && barreira [index2Click][1] == null){
					...
					if (retornar != 0)
						...
						System.out.printf("Entrou else chaca barreira \n");

						index1Click = checaSeBarreira(peca.CoordX,  peca.CoordY); //ver se -1
						int index1ClickS = checaSeSaida(peca.CoordX,  peca.CoordY);
						int index1ClickA = checaSeAbrigo(peca.CoordX,  peca.CoordY);
						System.out.printf("index1Clisck %d\n",index1Click);

						System.out.printf("index1ClisckS %d\n",index1ClickS  );
						System.out.printf("Peca coord x %d\n", peca.CoordX);
						System.out.printf("Peca coord y %d\n", peca.CoordY);
						
						if (index1ClickS != -1) {
							if (index1ClickS != -1 && saida[index1ClickS][0] == peca ){
								System.out.printf("Entrou else chaca barreira caso saida \n");
									if (saida[index1ClickS][1] != null ) {
							//	index1Click = checaSeSaida(peca.CoordX,  peca.CoordY);
								
										saida[index1ClickS][0] = saida[index1ClickS][1];
										saida[index1ClickS][0].alteraTam = false;
										saida[index1ClickS][1] = null;
									}
									else
										saida[index1ClickS][0] = null;
							}
						}
						if (index1ClickA != -1) {
							if (index1ClickA != -1 && abrigo[index1ClickA][0] == peca ){
								System.out.printf("Entrou else chaca barreira caso Abrigo \n");
									if (abrigo[index1ClickA][1] != null ) {
							//	index1Click = checaSeSaida(peca.CoordX,  peca.CoordY);
								
										abrigo[index1ClickA][0] = abrigo[index1ClickA][1];
										abrigo[index1ClickA][0].alteraTam = false;
										abrigo[index1ClickA][1] = null;
									}
									else
										abrigo[index1ClickA][0] = null;
							}
						}
						...
						if(index1Click != -1) {
							if (barreira[index1Click][0] == peca && barreira[index1Click][1] != null ) {
								System.out.printf("Entrou if cehca barreira\n");
							...
							else if (barreira[index1Click][1] == null ) {
								System.out.printf("Entrou else if cehca barreira\n");

								barreira[index1Click][0] = null;
							}
						}
					}
				}
	...
	//	System.out.printf("Peca selecionada = %d\n", ind);
		...
		if (captura == true)
			...
			caminhos = roll;
		Movimentacao.MovimentacaoNormal(x,  y, caminhos, cor, ind);
		captura = false;
	...
	public void segundoClick (int x, int y, int id) {
		...
		//	System.out.printf(" ---- PEAO %d EH ABRIGO = %s  \n",ind,Tabuleiro.times.elementAt(id).peoes.elementAt(ind).alteraTam);
			
			Tabuleiro.times.elementAt(id).peoes.elementAt(ind).qtdCasasAndadas += roll;
			int andadas = Tabuleiro.times.elementAt(id).peoes.elementAt(ind).qtdCasasAndadas;
			System.out.printf(" Quantidade de casas andadas = %d \n", andadas );
			
			
			movimento(Tabuleiro.times.elementAt(id).peoes.elementAt(ind), x, y, roll);
			if (andadas == 56) {
				Tabuleiro.times.elementAt(id).qtdPinosCasaFinal ++;
				//Tabuleiro.fimJogo(calculaFinalJogo()); teste jpanel com colocacoes
				if (Tabuleiro.times.elementAt(id).qtdPinosCasaFinal == 4)
					Tabuleiro.fimJogo(calculaFinalJogo());
			}
	...
	protected void confereMovimento(int x, int y) {
		...
		if (selecionado == false ) { //ainda nao selecionou peca para mover
			...
			if(passar != -1){
				...
				if ( Movimentacao.casaX > 6 && Movimentacao.casaX != 14 && Movimentacao.casaY == 7 ||
						Movimentacao.casaX == 7  && Movimentacao.casaY != 14 && Movimentacao.casaY > 6 ||
						Movimentacao.casaX < 8  && Movimentacao.casaY != 0 && Movimentacao.casaY == 7 ||
						Movimentacao.casaX == 7  && Movimentacao.casaY < 14 && Movimentacao.casaY != 0 ) {
					
					System.out.printf("--Tirou mais do que deveria \n");
					concluiuJogada = true;
					Menu.b4.setEnabled(true);
				}
				...
				else if (Movimentacao.casaX == x  && Movimentacao.casaY == y)
					segundoClick (x, y, passar);
			}
			selecionado = false; 
			jogadaNormal = false;
			if(concluiuJogada && captura == false){
				if (roll != 6) {
	...
	public static int numeroDado(int n) {
	    ...
	    if(i != -1){
	    	...
	    	if(saida[i][0] != null && saida[i][0].corP == corEquipedaVez || saida[i][1] != null && 
	    	    			saida[i][1].corP == corEquipedaVez){
	    		if (roll == 6)
	    			contaSeis();
	    		
	    	 	Tabuleiro.fundo = corEquipedaVez;
			    System.out.printf("entrou if aqui\n");
			    jogadaNormal = true;
			    Menu.b4.setEnabled(false);
	    	}else{
	    		int aux;
	    		...
	    		if(aux != -1){
	    			...
	    			//	System.out.printf("Peca de indice %d da equipe %s fora da casa inicial\n",aux, getEquipedaVez());
			    		...
			    		if (roll == 6 && tirou6 != 2) { // caso 3 x 6
			    			...
			    			//System.out.printf("Entrou conta 6 do dado\n");
			    			contaSeis();
			    		}
				    		
				    	else if(roll == 6 && tirou6 == 2){
				    		contaSeis();
				    		//System.out.printf("Tirou 6 pela 3 vez dado\n");
				    		caso3Seis(i);
				    	}
	    			}
	    		}else{ 
	...
	public static int jogadaAutomatica(int i) {
		...
		if (indice != -1) {
			...
			int val = checkMovimentAndUpdate(x, y, aux.elementAt(indice));

			if (val == 0){
				System.out.printf("entrouu \n");
				System.out.printf("checa true = %s \n", aux.elementAt(indice).alteraTam);
				movimento(aux.elementAt(indice), x, y, roll);

				return 0;
			}
			else
				System.out.printf("ERRO JOGADA AUTOMATICA \n");			
		}
		if (indice == -1 )
	...
	public String calculaFinalJogo() {
		
		String colocacoes = " ";
		int vermelho = 0, azul =0, ama = 0, verde=0;
		int posicoes[] = new int [4];
		boolean vermelhob = false, verdeb =false, amarb=false, azulb = false;
		
		for (int i =0 ; i<4 ; i++) {
			
			vermelho += Tabuleiro.times.elementAt(0).peoes.elementAt(i).qtdCasasAndadas;
			verde += Tabuleiro.times.elementAt(1).peoes.elementAt(i).qtdCasasAndadas;
			ama += Tabuleiro.times.elementAt(2).peoes.elementAt(i).qtdCasasAndadas;
			azul+= Tabuleiro.times.elementAt(3).peoes.elementAt(i).qtdCasasAndadas;
		}
		//total andado pelas 4 pecas = 224
		
		int restoVermelho = 224 - vermelho;
		int restoVerde = 224 - verde;
		int restoAma = 224 - ama;
		int restoAzul = 224 - azul;
		
		System.out.printf("FUNCAO Calcula Final vermelho = %d\n", restoVermelho);
		System.out.printf("FUNCAO Calcula Final verde = %d\n", restoVerde);
		System.out.printf("FUNCAO Calcula Final amarelo = %d\n", restoAma);
		System.out.printf("FUNCAO Calcula Final azul = %d\n", restoAzul);
		
		posicoes[0] = restoVermelho;
		posicoes[1] = restoVerde;
		posicoes[2] = restoAma;
		posicoes[3] = restoAzul;
		
		//ordena vetor
		 int cont1, cont2,aux;  
         for(cont1 =0; cont1<4; cont1++){  
             for(cont2 =0; cont2 <4; cont2++){  
                if(posicoes[cont1] < posicoes[cont2]){  
                     aux = posicoes[cont1];  
                     posicoes[cont1] = posicoes[cont2];  
                     posicoes[cont2] = aux;  
                 }  
             }  
         }   
         ...
         for (int i =0 ; i<4 ; i++) {
        	 ...
        	 if (posicoes[i] == restoVermelho && vermelhob == false) {
        		 int aux2 = i+1;
        		 colocacoes += aux2;
        		 colocacoes += " - Vermelho\n";
        		 vermelhob = true;
        	 ...
        	 else if (posicoes[i] == restoVerde && verdeb == false){
        		 int aux2 = i+1;
        		 colocacoes += aux2;
        		 colocacoes += " - Verde\n";
        		 verdeb= true;
        	 }
        	 else if (posicoes[i] == restoAma && amarb == false){
        		 int aux2 = i+1;
        		 colocacoes += aux2;
        		 colocacoes += " - Amarelo\n";
        		 amarb = true;
        	 }
        	 else if (posicoes[i] == restoAzul && azulb == false){
        		 int aux2 = i+1;
        		 colocacoes += aux2;
        		 colocacoes += " - Azul\n";
        		 azulb = true;
